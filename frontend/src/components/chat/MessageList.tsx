                ...
                {msg.role === 'ASSISTANT' && msg.sources && msg.sources.length > 0 && (
                  <div className="mt-4 pt-3 border-t border-gray-700/50">
                    <details className="group">
                      <summary className="flex items-center cursor-pointer list-none text-xs font-semibold text-gray-400 hover:text-gray-300 transition-colors uppercase tracking-wider">
                        <svg className="w-3.5 h-3.5 mr-1.5 transition-transform group-open:rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {msg.sources.map((source, i) => (
                          <div key={i} className="flex flex-col bg-[#111318] border border-gray-700/60 rounded-lg p-2.5 shadow-sm min-w-[200px] flex-1">
                            <span className="text-xs font-medium text-gray-200 truncate mb-1 flex items-center">
                              <svg className="w-3.5 h-3.5 mr-1.5 text-blue-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div className="flex items-center justify-between mt-1 text-[10px] text-gray-500 font-medium">
                              <span className="bg-gray-800/60 px-1.5 py-0.5 rounded">Page {source.pageNumber}</span>
                              <span className="text-blue-400/70">{(source.similarityScore * 100).toFixed(1)}% match</span>
                            </div>
                          </div>
                        ))}
                      </div>
